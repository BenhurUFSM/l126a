...
Em um registro, pode-se colocar quantos dados quiser, cada um deles podendo ser de qualquer dos tipos da linguagem (inclusive ponteiros, vetores e registros).
...
Para evitar ter que escrever isso tudo ao se declarar outras variáveis do mesmo tipo, ou declarar funções que recebam ou retornem dados desse tipo, geralmente se cria um nome alternativo ou um sinônimo para esses tipos.
Existem duas formas de se dar nomes a estruturas em C:
- com etiquetas
- com *typedef*

Com "etiquetas" (*tag* em inglês), coloca-se, entre `struct` e o `{` uma "etiqueta", que é um nome para o registro. 
A partir daí, pode-se usar `struct nome` sempre que precisar se referir a esse tipo.
Por exemplo, a definição do tipo de registro usado para representar a data de nascimento acima poderia receber o nome `data`, e ser usada para declarar mais variáveis desse mesmo tipo:
```c
  // cria o registro "struct data"
  struct data {
    int dia;
    int mes;
    int ano;
  ...
  // cria variáveis desse tipo
  struct data nascimento;
  struct data hoje;
```

Com `typedef`, pode-se criar um novo nome para um tipo de dados.
...
```c
  ...
  inteiro xis;
```
Pode-se usar o *typedef* com *structs* para dar um novo nome a um tipo que seja um registro.
...
#### Registros como parâmetro e retorno de funções
...
A função que recebe um registro como parâmetro, quando altera esse parâmetro está alterando a sua cópia, e não a variável da função chamadora.
...
```c
int dias_desde_ano_novo(data hoje)
{
  // vetor com o número de dias desde o início do ano até o início de cada mês
  ...
  n_dias = dias_antes[hoje.mes - 1];
  //   sem esquecer que os bissextos têm um dia a mais em fevereiro
  if (bissexto(hoje.ano) && hoje.mes > 2) n_dias++;
  // soma o dia da data
  n_dias += hoje.dia;
  return n_dias;
}
...
```
Como mostrado no exemplo acima, uma variável do tipo registro pode ser inicializada com os valores dos campos separados por vírgula, entre chaves. Os valores devem estar na ordem em que aparecem na definição da estrutura.
...
```c
int calcula_idade(data nascimento, data referencia)
{
  int idade = referencia.ano - nascimento.ano;
...
{
  float base = r.superior_direito.x - r.inferior_esquerdo.x;
  float altura = r.superior_direito.y - r.inferior_esquerdo.y;
  return base * altura;
}
...
Então, para acessar o campo `dia` da variável `d1` através do ponteiro `p`, parece que deveria ser 
    ```c
    *p.dia
    ```
Infelizmente não é, porque nessa expressão temos dois operadores, o operador `*` e o operador `.`, e o operador `.` tem precedência sobre o operador `*`.
A expressão `*p.dia` significa `*(p.dia)` e nós precisamos de `(*p).dia`.

A notação `*(p.dia)` significa: 

   `p` é uma estrutura, que contém um campo chamado `dia`; esse campo é um ponteiro, e eu quero dereferenciar esse ponteiro

É um significado perfeitamente válido, e bastante usado.
O outro significado, que é o que queremos, é:

   `p` é um ponteiro para uma estrutura que tem um campo chamado `dia` e eu quero acessar esse campo

Esse significado também é bastante usado, mas necessita dos parênteses.

Para evitar o desgaste dos dedos dos programadores com a digitação excessiva de parênteses, criou-se uma sintaxe alternativa para esse caso, com um novo operador, `->`, que tem à sua esquerda um ponteiro para estrutura e à sua direita o nome de um campo dessa estrutura.
A notação `p->dia` é equivalente a `(*p).dia`, e é bem mais comum.
Como exemplo, a função que vimos anteriormente poderia, em vez de retornar a estrutura `data`, receber um ponteiro para uma estrutura a ser alterada, e retornar um `bool` para informar que a data não fica no mesmo ano.

```c
void data_desde_ano_novo(int ano, int dias, data *pdata)
{
  // se recusa a calcular se tem mais dias do que cabe no ano
  if (dias > dias_no_ano(ano)) {
    return false;
  }
...
```c
  ...
  void calcula_media(aluno *a)
  {
    ...
    // calcula a média entre trabalhos e provas
    float media_trab = soma_trab / 5;
    float media_provas = (a->notas_provas[0] + a->notas_provas[1]) / 2;
    float media = (media_trab + media_provas * 2) / 3;
    // ajusta a média de acordo com a avaliação subjetiva do aluno
    media += (rand() % 200) / 100.0 - 1;
    // verifica se tem exame
    if (media < 7) {
       media_final = (media + a->nota_exame) / 2;
    } else {
       media_final = media;
    }
  }
...
1. Faça uma função que recebe o tamanho de um vetor e o vetor de alunos desse tamanho (com aluno declarado como acima), e imprime os nomes dos alunos, com um, cada um precedido por um 'A' ou por um 'R', caso tenha sido aprovado ou reprovado. Ao final, imprima o percentual de aprovados. Considere que as notas já foram calculadas.
1. Faça uma função que recebe um círculo e um ponteiro para um ponto. Se o ponto estiver dentro do círculo ou sobre sua borda, a função não faz nada. Caso contrário, o ponto deve ser alterado para que fique sobre a borda do círculo, onde a linha que une o centro do círculo com o ponto cruza essa borda.
2. Crie um tipo de dados que representa um alvo no espaço 2D. Um alvo tem um centro e 5 raios de valor crescente, representando 5 círculos concêntricos.
3. Faça uma função que recebe um alvo como declarado no exercício acima, e um ponto, que representa o local no espaço 2D onde uma flecha foi cravada. A função deve retornar o número de pontos obtidos com essa flechada: 100 se foi no círculo central e 0 se foi fora do círculo externo. Se entre esses dois, um círculo vale 15 pontos menos que o círculo imediatamente menor que ele. Caso a flecha esteja exatamente na fronteira, vale a pontuação menor.
